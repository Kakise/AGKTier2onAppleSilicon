...
static CURLcode connect_prep(struct connectdata *conn, int sockindex)
{
  ...
  */
  ...
  else
  {
    ...
    ssl_obj->len = strlen(szCABundle); 
	ssl_obj->buf = (uint8_t*)malloc(ssl_obj->len);
	memcpy( ssl_obj->buf, szCABundle, ssl_obj->len );
	if( ssl_obj_PEM_load(ssl_ctx, SSL_OBJ_X509_CACERT, ssl_obj, NULL) != SSL_OK )
	{
		ssl_obj_free(ssl_obj);
		infof(data, "error reading szCABundle string\n");
		if(SSL_CONN_CONFIG(verifypeer)) {
			return CURLE_SSL_CACERT_BADFILE;
		}
	}
	...
	{
		ssl_obj_free(ssl_obj);
		infof(data, "found certificates in szCABundle\n");
	}
  }
  ...
  SSL_EXTENSIONS *extensions = ssl_ext_new();
  extensions->host_name = conn->host.rawalloc;
  extensions->max_fragment_size = 0;
  ...
  if(SSL_SET_OPTION(primary.sessionid)) {
    ...
    if(!Curl_ssl_getsessionid(conn, (void **) &ssl_sessionid, &ssl_idsize,
      ...
      ssl = ssl_client_new(ssl_ctx, conn->sock[sockindex], ssl_sessionid, (uint8_t)ssl_idsize, extensions);
    }
...
static CURLcode connect_finish(struct connectdata *conn, int sockindex)
{
  ...
  if(SSL_CONN_CONFIG(verifypeer)) {
    int retSSL = ssl_verify_cert(ssl);
    if(retSSL != SSL_OK) {
      Curl_axtls_close(conn, sockindex);
	  failf(data, "server cert verify failed: %d", retSSL);
      return CURLE_PEER_FAILED_VERIFICATION;
    }
  }
  else
 ...
 *done = FALSE;
  ...
  if(conn->ssl[sockindex].connecting_state == ssl_connect_2) {
    ...
    if(ssl_handshake_status(conn->ssl[sockindex].ssl) != SSL_OK) {
	  ...
	  uint8_t would_block = 0;
      do 
	  {
        ssl_fcn_return = ssl_read(conn->ssl[sockindex].ssl, NULL, &would_block);
        if(ssl_fcn_return < 0) 
		{
          Curl_axtls_close(conn, sockindex);
...

{
  ...
  while(ssl_handshake_status(ssl) != SSL_OK) {
    ...
    ssl_fcn_return = ssl_read(ssl, NULL, 0);
    if(ssl_fcn_return < 0) {
      Curl_axtls_close(conn, sockindex);
      ssl_display_error(ssl_fcn_return); /* goes to stdout. */
      return map_error_to_curl(ssl_fcn_return);
  ...
  if(connssl->ssl) {
    ...
    if(what > 0) {
      /* Something to read, let's do it and hope that it is the close
         ...
         free_ssl_structs. */
      nread = (ssize_t)ssl_read(connssl->ssl, &buf, 0);
                          ...
                          CURLcode *err)
{
  ...
  *err = CURLE_OK;
  if(connssl) {
    ret = ssl_read(connssl->ssl, &read_buf, 0);
    if(ret > SSL_OK) {
