...
std::vector<Ref<ResultPoint> > WhiteRectangleDetector::detect() {
  ...
  if (!sizeExceeded && atLeastOneBlackPointFoundOnBorder) {
    ...
    for (int i = 1; i < maxSize; i++) {
      z = getBlackPointOnSegment(left, down - (float)i, left + (float)i, down);
      if (z != NULL) {
        break;
      }
    }
    ...
    //go down right
    for (int i = 1; i < maxSize; i++) {
      t = getBlackPointOnSegment(left, up + (float)i, left + (float)i, up);
      if (t != NULL) {
        break;
      }
    }
    ...
    //go down left
    for (int i = 1; i < maxSize; i++) {
      x = getBlackPointOnSegment(right, up + (float)i, right - (float)i, up);
      if (x != NULL) {
        break;
      }
    }
    ...
    //go up left
    for (int i = 1; i < maxSize; i++) {
      y = getBlackPointOnSegment(right, down - (float)i, right - (float)i, down);
      if (y != NULL) {
        break;
      }
    }
