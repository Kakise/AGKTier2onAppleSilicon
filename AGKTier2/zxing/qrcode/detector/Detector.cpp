...
#include <qrcode/detector/Detector.h>
#include <qrcode/detector/FinderPatternFinder.h>
#include <qrcode/detector/FinderPattern.h>
#include <qrcode/detector/AlignmentPattern.h>
#include <qrcode/detector/AlignmentPatternFinder.h>
...
float Detector::calculateModuleSize(Ref<ResultPoint> topLeft, Ref<ResultPoint> topRight, Ref<ResultPoint> bottomLeft) {
  // Take the average
  if ( custom_isnan( topLeft->getX() ) ) return 0;
  if ( custom_isnan( topLeft->getY() ) ) return 0;
  if ( custom_isnan( topRight->getX() ) ) return 0;
  if ( custom_isnan( topRight->getY() ) ) return 0;
  if ( custom_isnan( bottomLeft->getX() ) ) return 0;
  if ( custom_isnan( bottomLeft->getY() ) ) return 0;
...
float Detector::calculateModuleSizeOneWay(Ref<ResultPoint> pattern, Ref<ResultPoint> otherPattern) {
  ...
  if ( custom_isnan(moduleSizeEst1) ) {
    return moduleSizeEst2;
  }
  if ( custom_isnan(moduleSizeEst2) ) {
