...
#define BOOST_FOREACH(item, container)                      \
	if(assimpboost::foreach_detail::auto_any_base const& foreach_magic_b = assimpboost::foreach_detail::begin(container)) {} else       \
    if(assimpboost::foreach_detail::auto_any_base const& foreach_magic_e = assimpboost::foreach_detail::end(container))   {} else       \
    for(;!assimpboost::foreach_detail::done(foreach_magic_b,foreach_magic_e,container);  assimpboost::foreach_detail::next(foreach_magic_b,container))   \
        if (bool ugly_and_unique_break = false) {} else							\
        for(item = assimpboost::foreach_detail::deref(foreach_magic_b,container); !ugly_and_unique_break; ugly_and_unique_break = true)
