...
namespace AGK
{
	...
	class AGKVector
	{
		public:
			...
			AGKVector& MultPerspective( const AGKMatrix4 &mat );
	...
	class AGKVector4
	{
		public:
			...
			void Set( float vx, float vy, float vz, float vw ) { vec.Set(vx,vy,vz); w = vw; }
	...
	class AGKQuaternion
	{
		public:
			...
			float LengthSqr() { return w*w + x*x + y*y + z*z; }
			void Normalize();
			...
			void LookAtDiscrete( float vx, float vy, float vz, float roll, float limit );
	...
	class AGKMatrix3
	{
		public:
			...
			void MakeFromAxes( AGKVector &x, AGKVector &y, AGKVector &z );
	...
	class AGKMatrix4
	{
		public:
			...
			void MakeProjOffCenter( int iOrtho, float fLeft, float fRight, float fTop, float fBottom, float fNear, float fFar );
