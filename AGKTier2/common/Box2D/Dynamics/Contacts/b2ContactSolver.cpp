...
b2ContactSolver::b2ContactSolver(b2ContactSolverDef* def)
{
	...
	m_staticIndexArray = def->staticIndexArray;
	m_contacts = def->contacts;
}

void b2ContactSolver::InitVariables()
{
	...
	for (int32 i = 0; i < m_count; ++i)
	{
		...
		if ( m_staticIndexArray && bodyA->m_type == b2_staticBody ) 
		{
			vc->indexA = m_staticIndexArray->FindBody( bodyA );
			pc->indexA = vc->indexA;
		}
		if ( m_staticIndexArray && bodyB->m_type == b2_staticBody ) 
		{
			vc->indexB = m_staticIndexArray->FindBody( bodyB );
			pc->indexB = vc->indexB;
		}
...
bool b2ContactSolver::SolveTOIPositionConstraints(int32 toiIndexA, int32 toiIndexB)
{
	...
	for (int32 i = 0; i < m_count; ++i)
	{
		...
		float32 mB = 0.0f; // changed in Box2D 2.3.2
		float32 iB = 0.0f;
