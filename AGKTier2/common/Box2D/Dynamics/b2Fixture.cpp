...
void b2Fixture::Create(b2BlockAllocator* allocator, b2Body* body, const b2FixtureDef* def)
{
	...
	m_localShapeMemory = def->localShapeMemory;
	if ( m_localShapeMemory ) m_shape = def->shape->Clone(allocator);
	else m_shape = def->shape; // AGK controls the shape memory
...
void b2Fixture::Destroy(b2BlockAllocator* allocator)
{
	...
	// if AGK doesn't control the shape memory
	if ( m_localShapeMemory )
	{
	...
	case b2Shape::e_polygon:
		{
			...
			for (int32 i = 0; i < s->GetVertexCount(); ++i)
			{
