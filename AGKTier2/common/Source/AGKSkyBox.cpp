...
AGKSkyBox::AGKSkyBox( int visible )
{
	...
	m_pSkyObject->SetDepthReadMode( AGK_DEPTH_LEQUAL );
	m_pSkyObject->SetDepthWrite( 0 );
	m_pSkyObject->SetDepthRange( 0.999f, 1.0f );
	m_pSkyObject->SetShadowReceiveMode( 0 );

	m_pNoSunShader = new AGKShader();
	m_pNoSunShader->MakeSkyBoxNoSunShader();

	m_pSunShader = new AGKShader();
	m_pSunShader->MakeSkyBoxSunShader();
