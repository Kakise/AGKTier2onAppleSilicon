...
{
	m_scaleFactor = 40.0;
	m_gravity = btVector3(0.0,-10.0,0.0);
	m_elapsedTime = 1.0 / 60.0;
}
...
void	DynamicsWorld::Initialize()
{
	...
	m_dynamicsWorld->getDispatchInfo().m_allowedCcdPenetration = 0.00001;
...
void DynamicsWorld::ApplyDefaultCcd( btRigidBody* body )
{
	if( body->getInvMass() > 0.0 && !body->isStaticOrKinematicObject() )
	{
		...
		body->setCcdMotionThreshold( diameter / 2.0 );
