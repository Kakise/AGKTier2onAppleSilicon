...
namespace CollisionShapes
{
	...
	btCollisionShape* Cone( btVector3 &size, int axis )
	{
		...
		if( axis == X_AXIS )
		{
			cone = new btConeShapeX( size.getY() / 2.0,  size.getX() );
		}
		else if( axis == Y_AXIS )
		{
			cone = new btConeShape( size.getX() / 2.0, size.getY() );
		}
		else
		{
			cone = new btConeShapeZ( size.getY() / 2.0, size.getZ() );
		}
		return cone;
	}
	...
	btCollisionShape* Capsule(btVector3 &size, int axis)
	{
		...
		if( axis == X_AXIS )
		{
			capsule = new btCapsuleShapeX( size.getY() / 2.0, size.getX() - size.getY() );
		}
		else if( axis == Y_AXIS )
		{
			capsule = new btCapsuleShape( size.getX() / 2.0, size.getY() - size.getX() );
		}
		else
		{
			capsule = new btCapsuleShapeZ( size.getY() / 2.0, size.getZ() - size.getY() );
		}
		return capsule;
	}
