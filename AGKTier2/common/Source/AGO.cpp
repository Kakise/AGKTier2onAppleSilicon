...
int AGO::ParseFindChunk( const char *szFilename, char* data, int index, char* chunkName, UINT maxLength )
{
...
int AGO::ParseChunkFloatArray( const char *szFilename, const char *szChunkName, char* data, int index, float* f, UINT elements )
{
	UINT index2 = 0;
	uString sFloat( "", 20 );
	while ( data[ index ] && data[ index ] != '}' )
...
int AGO::ParseChunkIntArray( const char *szFilename, const char *szChunkName, char* data, int index, int* i, UINT elements )
{
	UINT index2 = 0;
	uString sInt( "", 20 );
	while ( data[ index ] && data[ index ] != '}' )
		...
		else
		{
			...
			else
			{
				...
				else if ( strcmp( chunkName, "Mesh" ) == 0 )
				{
					...
					{
						for ( UINT i = 0 ; i < pObject->m_iNumMeshes-1; i++ ) pNewMeshes[ i ] = pObject->m_pMeshes[ i ];
			...
			else
			{
				...
				*/
				else if ( strcmp( chunkName, "VertexAttrib" ) == 0 )
				{
					// parse vertex attrib
					bool bSuccess = false;
					cVertexAttrib *pNewAttrib = new cVertexAttrib();
					index = ParseChunkVertexAttrib( szFilename, data, index, pMesh->m_iNumAttribVertices, pNewAttrib, bSuccess );					
					if ( index < 0 ) return -1;
					if ( bSuccess )
					{
						pMesh->m_iNumAttribs++;
						cVertexAttrib **pNewAttribs = new cVertexAttrib*[ pMesh->m_iNumAttribs ];
						if ( pMesh->m_pVertexAttribs )
						{
							for ( int i = 0 ; i < pMesh->m_iNumAttribs-1; i++ ) pNewAttribs[ i ] = pMesh->m_pVertexAttribs[ i ];
							delete [] pMesh->m_pVertexAttribs;
						}
						pMesh->m_pVertexAttribs = pNewAttribs;
						pMesh->m_pVertexAttribs[ pMesh->m_iNumAttribs-1 ] = pNewAttrib;
						if ( pNewAttrib->m_sName.CompareTo( "position" ) == 0 ) pMesh->m_iPosAttrib = pMesh->m_iNumAttribs-1;
					}
				}
				else if ( strcmp( chunkName, "Indices" ) == 0 )
				{
					pMesh->m_pRawIndices = new UINT[ pMesh->m_iNumRawIndices ];
...
int AGO::ParseChunkVertexAttrib( const char *szFilename, char* data, int index, int numVertices, cVertexAttrib *pAttrib, bool &bSuccess )
{
	bSuccess = true;
	char chunkName[ 31 ];
		...
		else
		{
			...
			else
			{
				...
				else
				{
					...
					{
						pAttrib->m_sName.SetStr( "                    " ); // resize the character buffer for speed
						index = ParseChunkString( szFilename, data, index, pAttrib->m_sName );
						if ( index < 0 ) return -1;
					}
					...
					else if ( strcmp( chunkName, "Normalize" ) == 0 )
					{
						...
						if ( index < 0 ) return -1;
						sType.Lower();
						if ( sType.CompareTo( "yes" ) == 0 ) pAttrib->m_bNormalize = true;
						else pAttrib->m_bNormalize = false;
					}
