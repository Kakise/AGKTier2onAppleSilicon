...
void AGKMatrix4::MakeProj( float fFov, float fAspect, float fNear, float fFar, float fWidth, bool righthand )
{
	float xmax, ymax;
	...
	if ( fFov == 0 )
	{
		...
		float modifier = righthand ? -1.0f : 1.0f;
	...
	{
		// horizontal FOV
		float fTanFOV = float(agk::Tan( fFov / 2.0f ));
		//xmax = fNear * fTanFOV;
		//ymax = xmax / fAspect;
		...
		Identity();
		mat[0][0] = 1.0 / fTanFOV; // fNear / xmax
		mat[1][1] = fAspect / fTanFOV; // fNear / ymax
